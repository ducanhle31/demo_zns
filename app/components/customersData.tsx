...
export const CustomerSelector = (props: ICustomerSelectorProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<CampaignTemplate[]>('http://10.10.51.16:3001/api/v1/config');
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const currentTemplate = data.find((template) => template.file_id === fileId);
    if (currentTemplate) {
      const allCustomerNames = currentTemplate.customers.map((customer) => customer.customers);
  ...
  return (
    <div className="p-4">
      <div className="mb-6">
        <label htmlFor="file-select" className="font-bold mb-2">Hãy lựa chọn tệp</label>
        <select
          id="file-select"
          value={selectedFile}
